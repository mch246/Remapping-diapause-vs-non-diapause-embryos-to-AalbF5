## Data retrieval
I [first downloaded the SRA toolkit](https://github.com/ncbi/sra-tools/wiki/02.-Installing-SRA-Toolkit) to my home directory on the GU hpc.
I tried to fetch the raw .sra reads from NCBI and convert them to fastqz files in a [single script](scripts/SRA_download.sh), but the second step in the script failed because the .sra files were put in individual directories rather than in a single SRA directory. 
...
After checking the array behaved as expected based on read counts, I zipped all files and [renamed](misc/rename_SRA_files.sh) them with biologically meaningful names.
## Trimmomatic
